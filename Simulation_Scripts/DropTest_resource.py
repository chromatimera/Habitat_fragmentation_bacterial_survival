...
class DropTest_resource_release(object):
    ...
    def run(self, step, spec_time, total_sim):
        ...
        df_bact_count = pd.DataFrame()    #df_total_mass??
            ...
            for i in range(len(droplet_list)):
               ...
               # print('total droplets', total_drop_nr)
                ...
                if total_drop_nr == 0:
                    ...
                    df_bact_count['{} {}'.format(nr_sim, total_drop_nr)] = nr_bact_each_ts
        ...
        columns = list(df_bact_count.columns)
        ...
        df_bact_count = df_bact_count.reindex(columns_sorted_joined, axis=1)
        ...
        for i in range(0, len(part_fact), 1):
           for j in range(0, total_sim, 1):
               k = i * total_sim + j
               total_nr_bact[:, i] += df_bact_count.iloc[:, k]
           #print('stop')
       ...
       ## save all df
        ...
        folder_name = 'output/RESOURCE_dropnr_{}_loading_{}_growth_{}_initialN_{}_abconc_{}_thresh{}_rmax{}_Km{}'.format(variables.droplet_list[-1], variables.loading, variables.growth, variables.initialN, AB_conc, MIC, Vmax,Km)
        ...
        pd.DataFrame(df_bact_count).to_csv('df_growth_{}_starting_nr_drops_{}_ABconc{}.csv'.format(growth, variables.droplet_list[-1], AB_conc), index=None)
        ...
        def plot_resource():
            #plot pop size vs time
            plt.figure(1)
            plt.plot( avg_nr_bact)



simulate = DropTest_resource_release()
simulate.run(step, spec_time, total_sim)
