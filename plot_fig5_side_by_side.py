...
for antib, c in zip(ab, colors):
    ...
    surv_fraction_transpose1 = surv_fraction_transpose1.set_index('RhoV', drop=True)

    surv_fraction_transpose2.index = surv_fraction_transpose2.index.map(int)
    surv_fraction_transpose2 = surv_fraction_transpose2.set_index('RhoV', drop=True)
    ...
    avg_nr_bact1.iloc[-1, 0:(len(part_fact1))].plot(yerr=error_nr_bact_N_tot1.iloc[-1, 0:len(part_fact1)].tolist(), c=c, ax=ax1, logx=True)  ### plot initial nr of bacteria
    avg_nr_bact2.iloc[-1, 0:(len(part_fact2))].plot(yerr=error_nr_bact_N_tot2.iloc[-1, 0:len(part_fact2)].tolist(),linestyle='dashed', c=c, ax=ax1, logx=True)
    ...
    surv_fraction_transpose2["Surv frac"].plot.line(yerr=surv_fraction_errors2, linestyle='dashed',  c=color_list[ind], ax=ax2, logx=True)  # color = 'orange')
...
ax4 = ax1.secondary_xaxis("top")
ax4.xaxis.set_ticks(xticks[::-1], labels=second_ticks[::-1])
ax4.set_xlabel(r'\bf{m (number of subvolumes)}')

ax1.legend(label_list, title=r'\bf{Antibiotic concentration in $\mu$g/mL}', loc='upper center', bbox_to_anchor=(0.5, 1.25), ncol=4, fancybox=True, shadow=True, fontsize= 'large')
