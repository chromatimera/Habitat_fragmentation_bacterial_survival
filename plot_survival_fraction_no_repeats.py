...
for ab, c, ind in zip(antib, color, range(len(antib))):
    ...
    for i in range(len(droplet_list)):
        ...
        ### read dataframes
        binary_df = pd.read_csv('output/dropnr_1000_loading_rand_growth_binary_initialN_5_abconc_{}_gr_0.01/initialN5_growthrate0.01_MIC1_totaldropnr{}_ABconc{}_dt1_loadingrand_growthbinary.csv'.format(ab,droplet_list[i],ab))
